...
public:
    ...
    ExposurePatch(CONST_PTR(ExposureT) exp, ///< Exposure of interest
        ...
        const_cast<geom::AffineTransform&>(_fromStandard) = standardWcs.linearizePixelToSky(sky) *
            expWcs.linearizeSkyToPixel(sky);
        const_cast<geom::AffineTransform&>(_toStandard) = expWcs.linearizePixelToSky(sky) *
