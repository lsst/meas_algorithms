...
class TestRefObjLoader(ingestIndexTestBase.ConvertReferenceCatalogTestBase,
    ...
    def test_properMotion(self):
        ...
        loader.applyProperMotions(cat_pm, self.epoch)
        ...
        # One year difference
        loader.applyProperMotions(cat_pm, self.epoch + 1.0*astropy.units.yr)
    ...
    def test_requireProperMotion(self):
        ...
        with self.assertRaisesRegex(RuntimeError, msg):
            _ = loader.loadSkyCircle(
                center,
                30.0*lsst.geom.degrees,
                filterName='a'
            ).refCat

        # Test that anyFilterMapsToThis works.

        msg = 'Could not find flux field'
        with self.assertRaisesRegex(RuntimeError, msg):
            _ = loader.loadSkyCircle(
                center,
                30.0*lsst.geom.degrees,
        ...
        loaderConfig = ReferenceObjectLoader.ConfigClass()
        loaderConfig.requireProperMotion = True
        loaderConfig.anyFilterMapsToThis = 'a'
        loader = ReferenceObjectLoader([dataRef.dataId for dataRef in self.datasetRefs],
                                       self.handles,
                                       config=loaderConfig)
