...
class Curve(ABC):
    ...
    def _check_cols(self, cols, table):
        for col in cols:
...
class DetectorCurve(Curve):
    ...
    def fromTable(self, table):
        self._check_cols(['wavelength', 'efficiency'], table)
        self.__init__(table['wavelength'], table['efficiency'], table.meta)
...
class AmpCurve(Curve):
    ...
    def fromTable(self, table):
        self._check_cols(['amp_name', 'wavelength', 'efficiency'], table)
        self.__init__(table['amp_name'], table['wavelength'], table['efficiency'], table.meta)
    ...
    def toTable(self):
        ...
        for amp_name, val in self.data.items():
            wavelength = numpy.concatenate(wavelength, val[0])
            efficiency = numpy.concatenate(efficiency, val[1])
            names = numpy.concatenate(names, numpy.full(val[0].shape, amp_name))
        return Table({'amp_name': names, 'wavelength': wavelength, 'efficiency': efficiency},
                     meta=self.metadata)
