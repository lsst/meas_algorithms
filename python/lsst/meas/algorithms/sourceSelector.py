...
class CullFromMaskedRegion(pexConfig.Config):
    ...
    with any of the mask bits in the `badMaskNames` list.  Namely used for a
    reference catalog for which the flag columns we would get from the
    measurement plugins do not exist.

    NOTE: In the context of reference objects, do not include EDGE in the
   `badMaskNames` list as that will remove all the reference objects outside
    the detector but within the pixelMargin (thus nulling the pixelMargin
    ...
    def apply(self, catalog, exposure):
        ...
        bitmask = exposure.mask.getPlaneBitMask(self.badMaskNames)
