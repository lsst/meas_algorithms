 ...
 */
lsst::afw::image::Image<PSF::Pixel>::Ptr pcaPsf::getImage(double const x, ///< column position in parent %image
                                                           double const y  ///< row position in parent %image
                                                          ) const {
    lsst::afw::image::Image<PSF::Pixel>::Ptr im(new lsst::afw::image::Image<PSF::Pixel>(getWidth(), getHeight()));
...
// \cond
namespace {
    bool b = PSF::registerMe<pcaPsf, lsst::afw::math::Kernel::Ptr>("PCA");
}
