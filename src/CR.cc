 ...
 */
    ...
    bool grow = false;
    LOGL_TRACE("algorithms.CR.trace2", "Removing initial list of CRs");
 ...
 */
    ...
    for (int i = 0; i != niteration && !too_many_crs; ++i) {
        LOGL_TRACE("algorithms.CR.trace1", "Starting iteration %d", i);
 ...
 */
    if (keep || too_many_crs) {
        ...
        if (true || nextra > 0) {
            grow = true;
            LOGL_TRACE("algorithms.CR.trace2", "Removing final list of CRs, grow = %d", grow);
            removeCR(mimage, CRs, bkgd, crBit, saturBit, badMask, debias_values, grow);
        }
/*
 ...
 */
        if (ngood > 0) {
            LOGL_TRACE("algorithms.CR.trace5", "Adopted min==%g at (%d, %d) (ngood=%d)",
